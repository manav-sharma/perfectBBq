...
class Uploadfile {
	 ...
	 * @return	bool
	 */
	public function do_upload($field = 'userfile',$crop="false")
	{
	...
	// Is $_FILES[$field] set? If not, no reason to continue.
        ...
        /***
         * If  croping paramenter is set for true
         **/
        if($crop)
        {
            $data = $this->data();
            $CI =& get_instance();
            $CI->load->view("lightbox",$data);
        }
		return TRUE;
	}
   ...
   /**
    * Method to resize image to a particular height
    */
   ...
   /**
    * Method to resize image to a particular width
    */
   ...
   /**
    * Method to resize image to a particular scale 
    * PS: $scale will be samll for making image small and large to make image large
    */
   ...
   /**
    * Method to resize image to a particular height and with
    * @param type $width(INT)
    * @param type $height(INT) 
    */
   ...
   /**
    * Method to save image after selection
    */
   public function cropByDimensions($dimensionsArray = array(),$image_path="")
   {
         
        extract($dimensionsArray);
        $image = $this->upload_path.$file;
        $file_name = explode(".jpg",$file);
        $new_filename = $this->upload_path.$file_name['0'].'-resized.jpg';
        $myImage = imagecreatefromjpeg($image);
        // copying the part into thumbnail
        $thumb = imagecreatetruecolor($width, $height);
        imagecopyresampled($thumb, $myImage, 0, 0, $x1, $y1, $width,$height, $width, $height);
        imagejpeg($thumb,$new_filename,95);
