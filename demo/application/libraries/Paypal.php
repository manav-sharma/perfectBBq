...
class Paypal{
    ...
    */
    ...
    public function init(){
        ...
        $this->apiLive = ($CI->config->item('Sandbox'))?false:true;
        $this->apiUsername = $CI->config->item('APIUsername');
        $this->apiPassword = $CI->config->item('APIPassword');
        $this->apiSignature = $CI->config->item('APISignature');
    ...
    public function SetExpressCheckout($paymentInfo=array()){ 
        ...
        $orderNum = '';
        if(isset($paymentInfo['Order']['number']))
            $orderNum = '&PAYMENTREQUESTNUMBER='.$paymentInfo['Order']['number'];
        ...
        $nvpstr='&AMT='.$amount.'&PAYMENTACTION='.$paymentType.'&CURRENCYCODE='.$currencyCode.'&RETURNURL='.$returnURL.'&CANCELURL='.$cancelURL.'&DESC='.$desc.'&QTY='.$quantity.$orderNum;
