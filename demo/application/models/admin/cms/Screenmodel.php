...
class Screenmodel extends CI_Model {
	...
	**/
	...
	function getDetails($id) {
		...
		$this->db->where("tbl_screen.cmsid", $id);
	...
	*/
	function deletePage($id) {  
		...
		if ($this->db->delete('tbl_screen') == True){ 
			return TRUE;
		} else {
			return FALSE;
		}
	...
	*/
	function status($id, $status) {
		...
		$this->db->where('cmsid IN (' . $id . ')', '', false);
		return $this->db->update('tbl_screen', $data);
	}
	...
	**/
	public function addpage() { 
		...
		$pagedetails = array(
			...
			'cmsVariable' 		=> strtolower(preg_replace('/\s+/', '', $this->input->post('txttileEng'))),  
		...
		if ($this->db->insert('tbl_screen', $pagedetails)) { 
			return true;
		}
		return $return;
	}
	...
	**/
	public function editPageData($usrId) {
		...
		if ($this->db->update('tbl_screen', $pagedetails, $where)) { 
			return TRUE;
		} else {
			return FALSE;
		}
	...
	**/
	function getActivePages() {
		...
		$this->db->where("tbl_screen.cmsStatus", '1');
