	...
	*/
	public function index() {
		...
		$orderingVar['sortBy'] = $data['sortBy'] = $sortBy;
            ...
            $array = array('cmsTitle', 'cmsContent', 'cmsStatus'); //'geoName',
			
            $likeConditionArray = compact($array);
			  
				 $customConditionVar=  array(  
						 array( 
						 	'tableWithCondition'		=> "(tbl_cms.cmsTitle like '%".'FieldValue'."%')",
							'fieldName'					=> 'txtTtile'
						),
						 array( 
						 	'tableWithCondition'		=> "(tbl_cms.cmsContent like '%".'FieldValue'."%')",
							'fieldName'					=> 'txtdes'
						),
						array(
	...
	*/
    function editpage($id='') {
		...
		$pageData = $this->pagesmodel->getDetails($id);
		    $pageInfo 	= array(
				"id"			=> $pageData->cmsid,
				"cmsTitle" 		=> $pageData->cmsTitle,
				"cmsContent"		=> $pageData->cmsContent,
