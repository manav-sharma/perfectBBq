...
class Recipe extends MY_Controller {
	...
	*/
    private function __exposrtCSV($dataArray) { 
        $array = array(array('Title (English)','Title (German)','Description (English)','Description (German)','Image','Category (English)','Category (German)','Status','Date Created'));
    ...
    function addRecipe() {
		...
		if (!empty($_POST)) { 
			...
			if ($this->form_validation->run()) { 
				...
				$recTempMode = (isset($postData['txtTemprature']) && $postData['txtTemprature'] != "") ? $postData['txtTemprature'] : 'no';
				$recThickMode = (isset($postData['txtThickness']) && $postData['txtThickness'] != "") ? $postData['txtThickness'] : 'no';
				if (!isset($postData['txtTemprature']) && !isset($postData['txtThickness'])) {
					$recPostData['recMin'] = $postData['recipeMin'];
					$recPostData['recSec'] = $postData['recipeSec'];
					$recTempMode = "no";
					$recThickMode = "no";
				}
				$recPostData['recTempMode'] = $recTempMode;
				$recPostData['recThickMode'] = $recThickMode;
				$recPostData['recImage'] = $filename;
				$recPostData['recGrilled'] = (isset($postData['txtGrilled'])) ? $postData['txtGrilled'] : 0;
				...
				if ($insert_id = $this->Recipemodel->addRecipe($recPostData)) {
					...
					//add recipe timing start
					if (!isset($postData['txtTemprature']) && !isset($postData['txtThickness'])) {
						$deleteRecipe = $this->Recipemodel->deleteRecipeTiming($insert_id);
						$timeArr = array();
						$timeArr['recTimeMin'] = $postData['recipeMin'];
						$timeArr['recTimeSec'] = $postData['recipeSec'];
						$timeArr['recTotalSec'] = ($postData['recipeMin'] * 60) + $postData['recipeSec'];
						$timeArr['recTemp'] = $postData['recipeTemp'];
						$timeArr['recBowlTemp'] = $postData['recipeBowlTemp'];
						$timeArr['recId'] = $insert_id;
						$timeArr['recTimeDateCreated'] = date('Y-m-d H:i:s');
						$insert_id = $this->Recipemodel->addRecipeTiming($timeArr);
					}
					//add recipe timing end
	...
	*/
    function editRecipe($id='') {
		...
		$allCat = $this->Categorymodel->allActiveCatgegory();
		$data['allCat'] = $allCat; 
		$recipeTimeInfo = $this->Recipemodel->getRecipeTimeDetailByRecipeId($id);
		...
		$recInfo 	= array(
			...
			"recMin" => $recipeData->recMin,
			"recSec" => $recipeData->recSec,
			"recTemp" => $recipeData->recTemp,
			"recBowlTemp" => $recipeData->recBowlTemp,
			"recImage" => $recipeData->recImage,
			"recStatus" => $recipeData->recStatus,
			"recGrilled" => $recipeData->recGrilled,
		);
		//echo "<pre>"; print_r($recInfo);die;
		$recId = $recipeData->recId;
		$data['detail'] = $recInfo;
		
		$recTempInfo = array();
		if ($recipeData->recTempMode == "no" && $recipeData->recThickMode == "no") {
			$recipeTempData = $this->Recipemodel->getRecipeTimeDetailByRecipeId($id);
			if (!empty($recipeTempData)) {
				$recTempInfo['recTemp'] = $recipeTempData->recTemp;
				$recTempInfo['recBowlTemp'] = $recipeTempData->recBowlTemp;
			}
		}
		$data['recTempInfo'] = $recTempInfo;
		...
		if (!empty($_POST)) {
			...
			if ($this->form_validation->run()) {
				...
				if ($res) { 
					...
					//add recipe timing start
					$oldRecMin = $recipeData->recMin;
					$postedRecMin = $postData['recipeMin']; 
					$oldRecSec = $recipeData->recSec;
					$postedRecSec = $postData['recipeSec'];
					if ((trim($oldRecMin) != trim($postedRecMin)) || (trim($oldRecSec) != trim($postedRecSec))) { //echo "1";die;
						$deleteRecipe = $this->Recipemodel->deleteRecipeTiming($recId);
						$timeArr = array();
						$timeArr['recTimeMin'] = $postData['recipeMin'];
						$timeArr['recTimeSec'] = $postData['recipeSec'];
						$timeArr['recTotalSec'] = ($postData['recipeMin'] * 60) + $postData['recipeSec'];
	...
	*/
	public function timing($id = '') {
		if ($this->session->userdata('logged_in') == TRUE) {
			...
			$data['recTempMode'] = $recipeData->recTempMode;
			$data['recThickMode'] = $recipeData->recThickMode;
			$orderingVar = array();
			//$paging = array();
            $joinTableVar = array();
            $whereConditionVar = array();
            ...
            $whereConditionVar = array('recId'=>$id);
			 ...
			 $customConditionVar=  array(  
				...
				array( 
					'tableWithCondition'		=> "(recBowlTemp ='FieldValue')",
					'fieldName'					=> 'recTimeBowlTemp'
				),
				array(
		...
		if (!empty($_POST)) {
			...
			if (trim($tempMode) == 'yes' && trim($thickMode) == 'yes') {
				foreach ($timeTitle as $tKey => $tVal) {
					...
					$timeArr['recBowlTemp'] = $postData['bowlTemp'][$tKey];
	...
	*/
    function editTiming($id='') {
		...
		if (!empty($_POST)) {
			...
			$timeArr['recBowlTemp'] = $postData['bowlTemp'];
	...
	*/
    private function __exportIntervalCSV($dataArray) { 
        $array = array(array('Title (English)','Title (German)','Description (English)','Description (German)','Time (Min:Sec)','Date Created'));
		 
        foreach ($dataArray as $key => $int) {
            $tempArray = array();
			$tempArray['Title (English)'] = $int->recIntervalTitleEng;
			$tempArray['Title (German)'] = $int->recIntervalTitleDutch;
			$tempArray['Description (English)'] = $int->recIntervalDescEng;
			$tempArray['Description (German)'] = $int->recIntervalDescDutch;
